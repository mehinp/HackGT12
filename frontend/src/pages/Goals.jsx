// src/pages/Goals.jsx - Simplified (removed score widgets + KPI grid)
...
const Goals = () => {
  ...
  // Fetch goals + ML data on mount
  useEffect(() => {
    Promise.all([fetchGoals(), fetchMLData()]).finally(() => setLoading(false))
  }, [])
  ...
  const handleViewChange = (view) => {
    ...
    if (mlData) {
      ...
      if (viewData) setChartData(viewData)
    }
  }

  const handleGoalCreated = () => {
    Promise.all([fetchGoals(), fetchMLData()])
  ...
  const handleRefreshData = async () => {
    ...
    try {
      ...
      await Promise.all([fetchGoals(), fetchMLData()])
    } catch (err) {
  ...
  const pageStyle = { padding: '1rem 0', position: 'relative' }
  const headerStyle = {
    display: 'flex',
  ...
  const actionButtonsStyle = { display: 'flex', gap: '1rem', flexWrap: 'wrap' }
  ...
  const viewButtonsStyle = { display: 'flex', gap: '0.5rem', marginBottom: '1rem' }
      ...
      <div style={pageStyle}>
        <div style={headerStyle}>
          <div style={actionButtonsStyle}>
            <Button variant="primary" icon="➕" onClick={() => setShowAddGoal(true)}>
              Add New Goal
            </Button>
          </div>
        </div>
        ...
        <div style={mainContentStyle}>
          <div style={loadingStyle}>Loading your goals and projections...</div>
        </div>

        <FloatingChatbotButton onClick={() => setShowChatbot(true)} context="goals" />
      </div>
    )
  }
    ...
    <div style={pageStyle}>
      ...
      {error && (
        <div style={errorStyle}>
          ⚠️ {error}
          <Button variant="outline" size="sm" onClick={handleRefreshData} style={{ marginLeft: '1rem' }}>
            Retry
          </Button>
        </div>
      )}
      ...
      {mlError && (
        <div style={warningStyle}>
          ...
          <Button variant="outline" size="sm" onClick={fetchMLData} style={{ marginLeft: '1rem' }}>
            Retry ML Data
          </Button>
        </div>
      )}
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <div style={viewButtonsStyle}>
              <button style={viewButtonStyle(selectedView === 'week')} onClick={() => handleViewChange('week')}>Week</button>
              <button style={viewButtonStyle(selectedView === 'month')} onClick={() => handleViewChange('month')}>Month</button>
              <button style={viewButtonStyle(selectedView === 'full_horizon')} onClick={() => handleViewChange('full_horizon')}>Full</button>
            </div>
          </div>
        ...
        {goals && goals.length > 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {goals.map((goal, index) => {
              ...
              return (
                <div key={index} style={{ backgroundColor: '#f8fafc', padding: '1.5rem', borderRadius: '0.75rem', border: '1px solid #e2e8f0' }}>
          ...
          <div style={{ textAlign: 'center', padding: '2rem', backgroundColor: '#f8fafc', borderRadius: '0.75rem', border: '1px solid #e2e8f0', marginTop: '1rem' }}>
            ...
            <p style={{ color: '#6b7280', marginBottom: '1.5rem' }}>
              {mlInsights
                ? `With your current savings of ${formatNumber(mlInsights.currentSavings)} and monthly income of ${formatNumber(mlInsights.incomeMonthly)}, our AI can help you create realistic goals.`
                : 'Our AI assistant can help you create personalized financial goals based on your situation.'
              }
            </p>
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              <Button variant="primary" onClick={() => setShowAddGoal(true)} icon="🎯">
                Create Goal Manually
              </Button>
              <Button variant="secondary" onClick={() => { setChatbotContext('goals'); setShowChatbot(true) }} icon="🤖">
                Get AI Help
              </Button>
            </div>
          </div>
      ...
      {showChatbot && (
        <EnhancedChatbotModal
          ...
          initialMessage={
            goals?.length === 0
              ? `I see you haven't set any financial goals yet! 🎯 ${
                  mlInsights
                    ? `With your current savings of ${formatNumber(mlInsights.currentSavings)} and monthly income of ${formatNumber(mlInsights.incomeMonthly)}, `
                    : ''
                }I can help you create your first goal. Would you like to start with an emergency fund, vacation savings, or something else?`
              : 'Let’s improve your goal plan with a few quick tweaks. Ready to optimize? 📊'
          }
          userData={{
